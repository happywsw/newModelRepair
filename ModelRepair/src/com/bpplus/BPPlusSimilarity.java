...
public class BPPlusSimilarity {
    ...
    public void deduceRelationMatrix(NetSystem net)
    {
        ...
        deduceCons(htRM, htCausalM, htConcurrM, arTrans, TaskRelation.CONCURRENCY);

        //fix --> involved non-free-choice to |-> if necessary
        ...
        for(Transition trans: arTrans)
        {
            ...
            for(Transition succ: arSucc)
            {
                ...
                if(htRM.get(succ).get(trans) == null)
                {
                    ...
                    else if(tr== TaskRelation.INDIRECTCAUSAL)
                        htRM.get(succ).put(trans, TaskRelation.INVERSEINDIRECTCAUSAL);
                    else {
						htRM.get(succ).put(trans, TaskRelation.INVERSEIMPLICTCAUSUAL);
					}
                }
            }
        }
        ...
        for(Transition transI: arTrans)
        {
            ...
            for(Transition transJ: alConcurr)
            {
                ...
                for(Transition st: alConflict)
                {
                    ...
                    if(htRM.get(transI).get(st) != TaskRelation.CONFLICT)
                    {
                        htRM.get(transI).put(transJ, TaskRelation.CONCURRENCY);
        ...
        for(Transition trans: arTrans)
        {
            ...
            for(Transition succ: arSucc)
            {
                ...
                if(htRM.get(succ).get(trans) == TaskRelation.DIRECTCAUSAL && htRM.get(trans).get(succ) == TaskRelation.DIRECTCAUSAL)
                {
                        htRM.get(succ).put(trans, TaskRelation.L2LOOP);
                        htRM.get(trans).put(succ, TaskRelation.L2LOOP);
        ...
        for(Place p: arPlaces)
        {
            ...
            for(Transition pred: alInput)
            {
                ...
                for (Transition succ: alOutput)
                {
                    ...
                    //fix the wrong --> to |->
                    if(!isDirect){
                        htRM.get(pred).put(succ, TaskRelation.IMPLICTCAUSAL);
        ...
        for(Place placeInput: arInput)
        {
            ...
            for(Transition inputTrans: arInputTrans)
            {
                ...
                if(transPred == inputTrans ||
                        rel == TaskRelation.CONCURRENCY ||
                        //added for non-free-choice net
                        rel == TaskRelation.DIRECTCAUSAL || rel == TaskRelation.INDIRECTCAUSAL)
                {
                    isFound = true;
                    break;
                }
            ...
            for(int j=i+1; j<arInput.length; j++)
            {
                ...
                for(Transition transI: arPredI)
                {
                    ...
                    for(Transition transJ: arPredJ)
                    {
                        ...
                        {
                            htRelI.put(transJ, TaskRelation.CONCURRENCY);
                            htRelJ.put(transI, TaskRelation.CONCURRENCY);
    ...
    public void convertOneValue( HashMap<String, HashMap<String, Integer>> net){
    	Iterator iterator = net.entrySet().iterator();
    	while(iterator.hasNext()){
    		...
    		while(iterator2.hasNext()){
    			...
    			map.put((String) entry2.getKey(), convertMulti2One((int) entry2.getValue()));
    		}
    		
    		net.put((String)entry.getKey(), map);
    		
    	}
    }
    
    public  HashMap<String, HashMap<String, Integer>> getRelationMatrix(NetSystem net){
    	extractFeatures(net);
    	
        HashMap<String, HashMap<String, Integer>> htRM_P = htMatricesTrans.get(net);
        convertOneValue(htRM_P);
...
//         
        ...
        float result = 0.0f;
        /*//1.2: calculate the intersection of RM_P and RM_Q
        Set<String> setCommon = hmPtoQ.keySet();
        for(String transP: setCommon)
        {
            for(String transQ: setCommon)
            {
                try
                {
                    int row = htRM_P.get(transP).get(transQ);
                    row = convertMulti2One(row);
                    row = (int) (Math.log(row) / Math.log(2));

                    int col = htRM_Q.get(transP).get(transQ);
                    col = convertMulti2One(col);
                    col = (int) (Math.log(col) / Math.log(2));
                ...
                {
                    System.out.println(transP + "-->" + transQ);
                    throw e;
                }
            }
        }
    ...
    {
    	if(rel > TaskRelation.L2LOOP)
    		rel = rel - TaskRelation.L2LOOP;
    	else if(rel < TaskRelation.L2LOOP && rel > TaskRelation.CONFLICT)
            rel = rel - TaskRelation.CONFLICT;
        else if(rel < TaskRelation.CONFLICT && rel > TaskRelation.CONCURRENCY)
            rel = TaskRelation.CONCURRENCY;
    ...
    private static int[] cntNumOfRelsComp(NetSystem netP, int[] nTotalRelBP, int[] narCau2All, int[] narRev2All, int[] narPar2All, int[] narExc2All, HashMap<Transition, HashMap<Transition, Integer>> htRM)
    {
        ...
        for(Node n1: alNodes)
        {
            ...
            for(Node n2: alNodes)
            {
                ...
                {
                    if(rel == TaskRelation.DIRECTCAUSAL)
                        narPar2All[0]++;
                    else if(rel == TaskRelation.INDIRECTCAUSAL)
                        narPar2All[1]++;
                    else if(rel == TaskRelation.INVERSEDIRECTCAUSAL)
                        narPar2All[2]++;
                    else if(rel == TaskRelation.INVERSEINDIRECTCAUSAL)
                        narPar2All[3]++;
                    else if(rel == TaskRelation.CONCURRENCY)
                        narPar2All[4]++;
                    else if(rel == TaskRelation.CONCURRENCY)
                        narPar2All[5]++;
                    else if(rel == TaskRelation.CONFLICT)
                        narPar2All[6]++;
                }
                ...
                {
                    if(rel == TaskRelation.INVERSEDIRECTCAUSAL)
                        narRev2All[0]++;
                    else if(rel == TaskRelation.INVERSEINDIRECTCAUSAL)
                        narRev2All[1]++;
                    else if(rel == TaskRelation.CONFLICT)
