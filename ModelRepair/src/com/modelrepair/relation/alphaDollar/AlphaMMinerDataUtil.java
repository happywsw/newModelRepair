...
public class AlphaMMinerDataUtil {
	...
	public static AlphaSharpData convertRelation(AlphaPPData alphaPPData, LogEvents leEvents) {
		...
		int size = alT_log.size();		//�?有的任务的size;
		...
		for (int i=0; i<size; i++)
			for (int j=0; j<size; j++)
			{
				...
				if ((rela & Relation.DIRECTCAUSAL) > 0)
					dmCausal.set(i, j, 1);
				if ((rela & Relation.CONCURRENCY) > 0)
...
//					continue;
				if (dmCausal.get(i, j)>0)
				{
					...
					tuI.addRela(taskNameJ, Relation.DIRECTCAUSAL);					
				}
			}
		//9.paral		
		for (int i=0; i<size; i++)
...
//					continue;
				if (dmParal.get(i, j)>0)		
				{
					...
					tuI.removePred(taskNameJ);		//TODO 没搞明白为啥不需要pred里面.因为不是before关系了哦！但是succ保留下了，因为succ关系当且仅当->才可�?
					tuI.addPara(taskNameJ);
					tuI.addRela(taskNameJ, Relation.CONCURRENCY);
				}
			}
		//10.aba, 没有考虑，因为aba的作用是寻找->和||,但是->和||已经在alpha#中找出来了，�?以不�?要这个关系了，呵�?,实际上在alpha#中也没法考虑这个关系�?
		//�?要�?�虑，因为alphasharp.invisible_1_49�?5个模型则算不出来。aba,bab的关系时a->b,b->a,a\||b
		for (int i=0; i<size; i++)
			for (int j=0; j<size; j++)
			{
				if (i == j)
				...
				if (dmCausal.get(i, j) >0 && dmCausal.get(j, i)>0 && dmParal.get(i, j) == 0)
				{
					String taskNameI = _alT_log.get(i);
					String taskNameJ = _alT_log.get(j);
					_rmRelation.addRelation(taskNameI, taskNameJ, Relation.L2LOOP);
					_rmRelation.addRelation(taskNameJ, taskNameI, Relation.L2LOOP);
				}
			}
		//11.logEvents 不要考虑�?
