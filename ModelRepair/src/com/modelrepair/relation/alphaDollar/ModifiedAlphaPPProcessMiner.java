...
public class ModifiedAlphaPPProcessMiner implements MiningPlugin {
	...
	ArrayList alL1L = new ArrayList(); // 长度�??1的循环中的任务，
	HashMap<String, ArrayList<String>> shortLoop = new HashMap<String, ArrayList<String>>();
	HashMap<String, ArrayList<String>> implict = new HashMap<String, ArrayList<String>>();
	ArrayList alT_prime = new ArrayList();
	ArrayList alT_I = new ArrayList();
	ArrayList alT_O = new ArrayList();
	HashMap<String, ArrayList<String>> directFollow = new HashMap<String, ArrayList<String>>();
	...
	boolean hasInvisibleTask = false;
	...
	public MiningResult mine(LogReader log) {
		clearAll();
		Progress progress = new Progress("Mining " + log.getFile().getShortName() + " using " + getName());
	 ...
	 * @param availableY
	 */
	private void getAvailableXY(Transition t, HashSet<String> availableX, HashSet<String> availableY,
			HashSet<String> availableA, HashSet<String> availableB) {
	...
	private void getInvTaskAXYB(PetriNet petrinet) {
		...
		for (Transition transition : petrinet.getTransitions()) {
			...
			for (AXYB axyb : invTaskAXYB) {
				...
				case AXYB.START:
					if (availableA.size() == 0) // 首先保证transition是一个start类型�??
					{
				...
				case AXYB.SKIP:
					...
					String aName = axyb.a;
					String bName = axyb.b;
					if (availableX.contains(xName) && availableY.contains(yName) && availableA.contains(aName)
	 ...
	 * //1.将it的前驱place添加到iti�?? for (Place p :
	 ...
	 * //2.将it的后继place添加到it�?? for (Place p : (HashSet<Place>)
	 * it.getSuccessors()) { if (petriNet.getEdgesBetween((Transition) iti,
	 * (Place) p).size() == 0) { petriNet.addEdge((Transition) iti, (Place) p);
	 * } } //3.axyb AXYB axybITI = invTaskAXYB.get(iti.getIdentifier()); AXYB
	 * axybIT = invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT);
	 * nCombined ++; } } else if (isInSucc) { boolean isCombined =
	 * isCombinable(it.getPredecessors(), iti.getSuccessors(),
	 * it.getPredecessors(), iti.getPredecessors()); if (isCombined) {
	 * //将it的内容添加到 //1.将it的前驱place添加到iti�?? for (Place p :
	 * (HashSet<Place>)it.getPredecessors()) { if
	 * (petriNet.getEdgesBetween((Place) p, (Transition) iti).size() == 0) {
	 * petriNet.addEdge((Place) p, (Transition) iti); } }
	 * //2.将it的后继place添加到it�?? for (Place p : (HashSet<Place>)
	 ...
	 * } } //3.axyb AXYB axybITI = invTaskAXYB.get(iti.getIdentifier()); AXYB
	 * axybIT = invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT);
	 * nCombined ++; } } else { boolean isCombined =
	 * isCombinable(iti.getPredecessors(), it.getSuccessors(),
	 * iti.getSuccessors(), it.getSuccessors()); isCombined = isCombined &&
	 * isCombinable(it.getPredecessors(), iti.getSuccessors(),
	 * it.getPredecessors(), iti.getPredecessors()); if (isCombined) {
	 * //将it的内容添加到 //1.将it的前驱place添加到iti�?? for (Place p :
	 * (HashSet<Place>)it.getPredecessors()) { if
	 ...
	 * } } //3.axyb AXYB axybITI = invTaskAXYB.get(iti.getIdentifier()); AXYB
	 * axybIT = invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT);
	 * nCombined ++; } } } if (nCombined > 0) { //4.删除it
	 * petriNet.delTransition(it); continue; } for (int j=i+1;
	 * j<rawInvisibleTask.size(); j++) { Transition iti =
	 * rawInvisibleTask.get(j); boolean isInPred =
	 * iti.getPredecessors().containsAll(dsPred); boolean isInSucc =
	 * iti.getSuccessors().containsAll(dsSucc); if (isInPred && isInSucc) {
	 * nCombined ++; } else if (isInPred) { boolean isCombined =
	 * isCombinable(iti.getPredecessors(), it.getSuccessors(),
	 ...
	 * axybIT = invTaskAXYB.get(it.getIdentifier()); axybITI.combine(axybIT);
	 * nCombined ++; } } } if (nCombined > 0) { //4.删除it
	 * petriNet.delTransition(it); } else if (nCombined == 0) {
	 * combinedInvisibleTask.add(it); } } }
	 */

	private boolean isCombinable(HashSet<Place> sarIPredH, HashSet<Place> sarSuccH, HashSet<Place> sarISuccH,
	 ...
	 * HashMap<HashSet<ModelGraphVertex>,HashMap<HashSet<ModelGraphVertex>,
	 * ArrayList<Transition>>>(); for (Transition transition :
	 * petrinet.getTransitions()) { if (transition.getLogEvent() != null)
	 * continue; HashSet<ModelGraphVertex> prePlaces =
	 * transition.getPredecessors(); HashSet<ModelGraphVertex> postPlaces =
	 ...
	 */
	/*
	 * 重构 adjustNonfreeChoice (对应的是split) 对于�??个不可见任务A,如何重构:
	 * 1.判断A中有多少组axyb�??要添加新的place,(无论是x或�?�y都算�??)，记录个数为num 1.1
	 * 如果num=0,那么不许要split 1.2 如果num=1,那么直接添加�??要的place即可, 1.3如果num>1,split
	 * (num-1)次，每一次分别添加所�??要的place
	 */
	...
	private ArrayList<AXYB> getNeedtoAjustAXYBs(ArrayList<AXYB> tAXYBs, IDWManager alID_W32) {
					...
					for (int m = 0; m < alGroupB.length; m++) {
						...
						if (rmRelation.isFitableAB(alA, alB)) {
							...
							for (int n = 0; n < alB.size(); n++) {
								...
								ArrayList alInputOfBj = rmRelation.calculateInput(bj);
						...
						for (int k = 0; k < alOrSplit.size(); k++) {
							...
							for (int l = 0; l < arB.length; l++) {
								...
								for (int m = 0; m < arY.length; m++) {
									String y = arY[m];
									// y || a or y->a or y>>a?
									if (!isExistA && rmRelation.isParaOrFollow(y, a)) {
										isExistA = true;
									}
									// y || b or y->b or y>>b?
									if (!isExistB && rmRelation.isParaOrFollow(y, b)) {
										isExistB = true;
									}
								...
								if (isExistA && !isExistB || !isExistA && isExistB) {
									String succ = "";
									if (isExistA && !isExistB) {
										succ = b;
						...
						for (int k = 0; k < alOrJoin.size(); k++) {
							...
							for (int l = 0; l < arB.length; l++) {
								...
								for (int m = 0; m < arY.length; m++) {
									String y = arY[m];
									// y || a or y->a or y>>a?
									if (!isExistA && rmRelation.isParaOrFollow(a, y)) {
										isExistA = true;
									}
									// y || b or y->b or y>>b?
									if (!isExistB && rmRelation.isParaOrFollow(b, y)) {
										isExistB = true;
									}
									if (isExistA && isExistB) {
										break;
	...
	private void convertIDW1() {
		...
		for (int i = 0; i < alID_W1.size(); i++) {
			...
			rmRelation.addRelation(id.getPred(), id.getSucc(), Relation.DIRECTCAUSAL);
	...
	private void unConvertIDW1() {
		...
		for (int i = 0; i < alID_W1.size(); i++) {
			ImplicitDependency id = (ImplicitDependency) alID_W1.get(i);
			rmRelation.removeRelation(id.getPred(), id.getSucc(), Relation.DIRECTCAUSAL);
	...
	private DoubleSets[] genInputPlaces(String t) {
		...
		for (int j = 0; j < arPred.length; j++) {
			...
			for (int k = 0; k < arSucc.length; k++) {
				String b = arSucc[k];
				if (!b.equals(t) && rmRelation.isDependent(b, t) && !alSucc.contains(b)) {
					alSucc.add(b);
				}
			}
		}
	...
	private ReachableManager getReachable() {
		getTypes();
	...
	private void induceReachable() {
		...
		ReachableManager rm = getReachable();
		...
		for (int i = 0; i < np && !rm.isFinished(); i++) {
			...
			// TODO 修改�?? 修改成为�??个符合条件的invisibleTask都要个来�??遍的那种�??
			...
			// ystem.o
			// rm.rm.getRelation(t1, t2)
			// printMatrix(rm.rm);
			rm.reset();
		}
	}

	public static void printMatrix(RelationMatrix matrix) {
		Enumeration keys = matrix.htRelations.keys();
		while (keys.hasMoreElements()) {
			String t = (String) keys.nextElement();
			TaskUnit tu = (TaskUnit) matrix.htRelations.get(t);
			String[] relations = tu.getRela();
			System.out.print("[" + t + "]: ");
			for (String str : relations) {
				System.out.print(str + ":" + matrix.getRelation(t, str) + "\t");
			}
			System.out.println();
		}
	}
	...
	private ArrayList<AuditTrailEntryList> adjustInvisibleTask(AuditTrailEntryList atel,
		...
		// 遍历atel中的每一个位置，判断这个位置是不是需要添加一个invisibleTask,如果�??要添加，那么�??要判断一下这个invisibletask.
		HashMap<Integer, ArrayList<InvTask>> posInvisibleTask = new HashMap<Integer, ArrayList<InvTask>>(); // each
																											...
																											// invisible
																											// tasks.
		// 1.判断�??有可能添加invisibleTask的位�??,存到posInvisibleTask里面
		try {
			// start
			...
			} else // there exisits matchings..
			{
				...
				getAllTraces(-1, 0, sortedPos, result, nowList, atel, posInvisibleTask); // 递归来找�??有的可能的trace�??
			}
		} catch (IndexOutOfBoundsException | IOException e) {
			e.printStackTrace();
		}
	...
	private void getAllTraces(int lastStep, int step, Integer[] sortedPos, ArrayList<AuditTrailEntryList> result,
			AuditTrailEntryList nowList, AuditTrailEntryList basicList,
			HashMap<Integer, ArrayList<InvTask>> posInvisibleTask) throws IndexOutOfBoundsException, IOException {
	 ...
	 * 
	 * if (pos == 0) //第一�?? { if (invTask.type != InvTask.START)
	 * //不是start的话，不予�?�虑 continue; if (sucTasks.contains(postLogName))
	 * //包含了，�??要添�?? { AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
	 * String invTaskName = invTask.taskName; int splitPos =
	 * invTaskName.indexOf("\0"); String taskElement = invTask.taskName; String
	 * taskType = "auto"; ate.setElement(taskElement); ate.setType(taskType);
	 * result.insert(ate, pos); }
	 * 
	 * } else if (pos+1 == result.size() && invTask.type == InvTask.END) { if
	 * (preTasks.contains(postLogName)) //包含了，�??要添�?? { AuditTrailEntryImpl ate
	 ...
	 * sucTasks.contains(postLogName)) //包含了，�??要添�?? { AuditTrailEntryImpl ate
	 * = new AuditTrailEntryImpl(); String invTaskName = invTask.taskName; int
	 * splitPos = invTaskName.indexOf("\0"); String taskElement =
	 * invTask.taskName; String taskType = "auto"; ate.setElement(taskElement);
	...
	private void computeTlogL1LTprime() {
		int np = log.numberOfInstances();
		for (int i = 0; i < np; i++) {
			ProcessInstance pi = log.getInstance(i);
			AuditTrailEntryList atel = pi.getAuditTrailEntryList();
			...
			for (int j = 0; j < atel.size(); j++) {
				try {
					...
					// find self-loop
					if ((task.equals(last1Task)) && (!this.alL1L.contains(task))) {
						alL1L.add(task);

					}
					// last1task: before the current task and distance is 1
					if ((!last1Task.equals("")) && (!last1Task.equals(task))) {
						this.rmRelation.addRelation(last1Task, task, Relation.DIRECTCAUSAL);
					}
					// find the shortloop
					if ((last2Task.equals(task)) && (!task.equals(last1Task))) {
						this.rmRelation.addRelation(last2Task, last1Task,
						 Relation.L2LOOP);
						if(!shortLoop.containsKey(last2Task))
							shortLoop.put(last2Task, new ArrayList<String>());
						shortLoop.get(last2Task).add(last1Task);
					}
					last2Task = last1Task;
					last1Task = task;
				} catch (IOException ex) {
			...
			if (!this.alT_O.contains(last1Task)) {
				this.alT_O.add(last1Task);
			}
		}

		// initMatrix(rmRelation, this.alT_log);

		// printMatrix(rmRelation);

		this.alT_prime.addAll(this.alT_log);
		this.alT_prime.removeAll(this.alL1L);
	}
	...
	public MiningResult mineMiscInfo(LogReader log, AlphaPPData alphaPPData, Progress progress) {
		// 1.把alphaPPData里面的变量赋值进�??
		alT_log = alphaPPData.alT_log;
		alL1L = alphaPPData.alL1L; // 长度�??1的循环中的任务，
	...
	// alpha++ mining relations
	public HashMap<String, HashMap<String, Integer>> mineAlphPPInfo(LogReader log, AlphaPPData alphaPPData, boolean flag) {
		this.log = log;
		alT_log = alphaPPData.alT_log;
		alL1L = alphaPPData.alL1L; // 长度1的循环中的任务，
		alT_prime = alphaPPData.alT_prime;
		alT_I = alphaPPData.alT_I;
		alT_O = alphaPPData.alT_O;
		rmRelation = alphaPPData.rmRelation;
		...
		HashMap<String, HashMap<String, Integer>> matrix = convertMatrix();
		return matrix;
	}

	private void reduceMultiRelation() {
		/*for(int i = 0; i < alT_log.size(); i++){
			for (int j = 0; j < alT_log.size(); j++) {
				if(rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) >= Relation.CONFLICT){
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.CONFLICT);
				}
				if(rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) >= Relation.IMPLICTCAUSAL
						&& rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) < Relation.INVERSEIMPLICTCAUSUAL)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.IMPLICTCAUSAL);
			}
		}*/
		for (int i = 0; i < alT_log.size(); i++) {
			for (int j = 0; j < alT_log.size(); j++) {
			if(rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) > Relation.CONFLICT){
				rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.CONFLICT);
				rmRelation.addRealRelation((String) alT_log.get(j), (String) alT_log.get(i), Relation.CONFLICT);
			}
			else if (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) > Relation.L2LOOP
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.DIRECTCAUSAL)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.L2LOOP);

				else if (rmRelation.getRelation((String) alT_log.get(i),
						(String) alT_log.get(j)) > Relation.DIRECTCAUSAL
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.INVERSEDIRECTCAUSAL)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.DIRECTCAUSAL);

				else if (rmRelation.getRelation((String) alT_log.get(i),
						(String) alT_log.get(j)) > Relation.INVERSEDIRECTCAUSAL
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.IMPLICTCAUSAL)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j),
							Relation.INVERSEDIRECTCAUSAL);

				
				else if (rmRelation.getRelation((String) alT_log.get(i),
						(String) alT_log.get(j)) >= Relation.IMPLICTCAUSAL
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.INVERSEIMPLICTCAUSUAL)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j),
							Relation.IMPLICTCAUSAL);

				else if (rmRelation.getRelation((String) alT_log.get(i),
						(String) alT_log.get(j)) > Relation.INVERSEIMPLICTCAUSUAL
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.CONCURRENCY)
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j),
							Relation.INVERSEIMPLICTCAUSUAL);

				else if (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) > Relation.CONCURRENCY
						&& rmRelation.getRelation((String) alT_log.get(i),
								(String) alT_log.get(j)) < Relation.CONFLICT) {
					rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.CONCURRENCY);
					rmRelation.addRealRelation((String) alT_log.get(j), (String) alT_log.get(i), Relation.CONCURRENCY);
	...
	private void deduceImplict() {
		// deduce implict dependency
		for (int i = 0; i < this.alID_W1.size(); i++) {
			ImplicitDependency id = (ImplicitDependency) this.alID_W1.get(i);
			if(!implict.containsKey(id.pred))
				implict.put(id.pred, new ArrayList<String>());
			implict.get(id.pred).add(id.succ);
		}

		Iterator iterator = alID_W2.htPred.entrySet().iterator();

		Iterator iterator2 = alID_W2.htSucc.entrySet().iterator();
		while (iterator2.hasNext()) {
			Entry entry = (Entry) iterator2.next();
			if(!implict.containsKey(entry.getValue()))
				implict.put((String) entry.getValue(), new ArrayList<String>());
			implict.get(entry.getValue()).add((String) entry.getKey());
		}

		Iterator iterator4 = alID_W3.htSucc.entrySet().iterator();
		while (iterator4.hasNext()) {
			Entry entry = (Entry) iterator4.next();
			ArrayList<String> list = (ArrayList<String>) entry.getValue();
			for (String str : list) {
				System.out.println(str + "->" + (String) entry.getKey());
				if(!implict.containsKey(str))
					implict.put(str, new ArrayList<String>());
				implict.get(str).add((String) entry.getKey());
			}
		}
	}

	public HashMap<String, HashMap<String, Integer>> convertMatrix() {
		HashMap<String, HashMap<String, Integer>> matrix = new HashMap<String, HashMap<String, Integer>>();
		for (int i = 0; i < alT_log.size(); i++) {
			if (((String) alT_log.get(i)).contains(Relation.SLIENTTRANSITION))
				continue;

			HashMap<String, Integer> values = new HashMap<String, Integer>();
			for (int j = 0; j < alT_log.size(); j++) {
				if (((String) alT_log.get(j)).contains(Relation.SLIENTTRANSITION))
					continue;
				values.put(((String) alT_log.get(j)).split("\0")[0],
						rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)));

			}
			matrix.put(((String) alT_log.get(i)).split("\0")[0], values);
		}
		return matrix;
	}
	
	public void deduceRelation(){
		for(int i = 0; i < alT_log.size(); i++){
			if (alL1L.contains(alT_log.get(i))){
				
				rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(i),
						Relation.DIRECTCAUSAL);
			}else{
				rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(i), Relation.CONFLICT);
			}
			
			if (shortLoop.size() != 0 && (shortLoop.containsKey(alT_log.get(i)))){
				for(String j : shortLoop.get(alT_log.get(i))){
					if(!((String)alT_log.get(i)).equals(j)){
					rmRelation.addRealRelation((String) alT_log.get(i), j, Relation.L2LOOP);
					rmRelation.addRealRelation(j, (String) alT_log.get(i), Relation.L2LOOP);
					}
				}
			}	
			if (implict.size() != 0  && (implict.containsKey(alT_log.get(i)))){
				for(String j : implict.get(alT_log.get(i))){
					if(!((String)alT_log.get(i)).equals(j)){
						System.out.println("no.no...............");
						
						rmRelation.addRealRelation( (String) alT_log.get(i), j, Relation.IMPLICTCAUSAL);
					}
				}
			}
	...
	private void findAdvRelation() {
		//if(hasInvisibleTask)
		computeTlogL1LTprime();
		
		deduceImplict();
		System.out.println("no..................");
		System.out.println("size:"+implict.size());
		...
		for (int i = 0; i < alT_log.size(); i++) {
			for (int j = 0; j < alT_log.size(); j++) {
				if(i == j)
					continue;
				if((rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) == 0 
						&& rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == 0)
						|| (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) == Relation.REACHABLE
								&& rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == 0)){
	
					rmRelation.addRelation((String)alT_log.get(i), (String)alT_log.get(j), Relation.CONFLICT);
					rmRelation.addRelation((String)alT_log.get(j), (String)alT_log.get(i), Relation.CONFLICT);
				
				}else if (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) != 0
							&& (rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == 0 
							|| (rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == Relation.CONFLICT 
							|| (rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == Relation.REACHABLE))))
						rmRelation.addRelation((String) alT_log.get(j), (String) alT_log.get(i),
						rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) + 2);
				}
			}
		}
		/*for (int i = 0; i < alT_log.size(); i++) {
			for (int j = 0; j < alT_log.size(); j++) {
				if (i == j)
					if (alL1L.size() != 0 && alL1L.contains(alT_log.get(i)))
						rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(i),
								Relation.DIRECTCAUSAL);
					else
						rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(i), Relation.CONFLICT);
					...
					if (shortLoop.size() != 0
							&& (shortLoop.containsKey(alT_log.get(i)) && shortLoop.containsValue(alT_log.get(j))))
						rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.L2LOOP);

					if (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) != 0
							&& (rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == 0 
							|| (rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == Relation.CONFLICT )))
						rmRelation.addRealRelation((String) alT_log.get(j), (String) alT_log.get(i),
								rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) + 2);

					else if (rmRelation.getRelation((String) alT_log.get(i),
							(String) alT_log.get(j)) > Relation.CONFLICT
							|| (rmRelation.getRelation((String) alT_log.get(i), (String) alT_log.get(j)) == 0
									&& rmRelation.getRelation((String) alT_log.get(j), (String) alT_log.get(i)) == 0)) {
						rmRelation.addRealRelation((String) alT_log.get(i), (String) alT_log.get(j), Relation.CONFLICT);
						rmRelation.addRealRelation((String) alT_log.get(j), (String) alT_log.get(i), Relation.CONFLICT);
					}
	...
	private void findL1L() {
		...
		// alL1L.clear();
		// TODO check each task in L1L, determine whether it should be removed
		// like alpha++ paper 1st of "not-done"
		checkL1L();
		alT_prime.clear();
	...
	private void checkL1L() {
		...
		for (String s : (ArrayList<String>) alL1L) {
			...
			// to apply with the therom 2, there should be the following
			// conditions:
			...
			for (String pre : pres)
				for (String succ : succs) {
					...
					ArrayList<String> commPreds = AlphaMMinerDataUtil.join(rmRelation.getBefore(pre),
							rmRelation.getBefore(succ));
					if (commPreds.size() == 0)
						continue;
					// 4.there should another task sharing com Pre with s,
					String[] preSuccs = rmRelation.getSucc(pre);
					if (preSuccs.length == 1) // 1 means the pre only has one
												// succ, namely the s.
						continue;
					// 5. iterate on the preSucc, check whether there a preSucc,
					// such as follows
					// 5.1 preSucc # s, (in order to make sure s and preSucc
					// share the same input place.)
					// 5.2 whether the presucc can be reached by pre, while the
					// s is not reachable with the d.
					boolean isExist = false;
					for (String preSucc : preSuccs) {
						...
						// TODO
						// we should check >> relation for each succ
						// preSucc..hehe
		...
		for (String s : (ArrayList<String>) alL1L) {
			...
			for (String pre : pres)
				for (String succ : succs) {
					...
					ArrayList<String> commSuccs = AlphaMMinerDataUtil.join(rmRelation.getSucc(pre),
							rmRelation.getSucc(succ));
					if (commSuccs.size() == 0)
						continue;
					// 4.there should another task sharing com Succ with s,
					String[] succPres = rmRelation.getBefore(succ);
					if (succPres.length == 1) // 1 means the pre only has one
												// succ, namely the s.
						continue;
					// 5. iterate on the succPre, check whether there a succPre,
					// such as follows
					// 5.1 succPre # s, (in order to make sure s and preSucc
					// share the same input place.)
					// 5.2 whether the succPre can reach to pre, while the s is
					// not reachable with the pre.
					boolean isExist = false;
	...
	private void adjustSharpRelation(ArrayList<String> pres, ArrayList<String> succs, String s) {
		for (String pre : pres)
			for (String succ : succs) {
					...
					if (!rm.isLoopPara(a, b)) {
						...
						for (int l = 0; l < alPlacesOfTc.size(); l++) {
							...
							if (ts.containedInA(a) && ts.containedInB(b)) {
								...
								for (int n = 0; n < arAprime.length; n++) {
									String a_prime = arAprime[n];
									if (rm.isLoopPara(a_prime, b) || !rm.isDependent(a_prime, a)) {
										isSatisfied = false;
										break;
									}
								}
								...
								for (int n = 0; n < arBprime.length; n++) {
									String b_prime = arBprime[n];
									if (rm.isLoopPara(b_prime, a) || !rm.isDependent(b_prime, b)) {
										isSatisfied = false;
										break;
									}
								}
			...
			for (int i = alForest.size() - 1; i >= 0; i--) {
				...
				if (isInA) {
					...
					for (int j = 0; j < arA.length; j++) {
						...
						if (rm.isNormSucc(arA[j], succ) && rm.isDependent(arA[j], pred)) {
							dsNew.addToA(arA[j]);
						}
					}
					String[] arB = ds.getB();
					for (int j = 0; j < arB.length; j++) {
						...
						if (rm.isNormSucc(pred, arB[j]) && rm.isDependent(succ, arB[j])) {
							dsNew.addToB(arB[j]);
						}
					}
				}
...
class IDWManager {
	...
	protected void convert(RelationMatrix rmRelation) {
		...
		while (keys.hasMoreElements()) {
			...
			for (int i = 0; i < alSucc.size(); i++) {
				String succ = (String) alSucc.get(i);
				rmRelation.addRelation(pred, succ, Relation.DIRECTCAUSAL);
...
class PlacesWithIDTwo {
	 ...
	 *            RelationMatrix
	 */
	protected void addNewComm(RelationMatrix rmRelation, ArrayList alOrSplit, ArrayList alOrJoin) {
		Enumeration keys = htPlaces.keys();
		while (keys.hasMoreElements()) {
			DoubleSets ds = (DoubleSets) keys.nextElement();
			String[] arA = ds.getA();
			...
			for (int i = 0; i < alID.size(); i++) {
				...
				if (ds.containedInA(id.pred)) {
					for (int j = 0; j < arB.length; j++) {
						rmRelation.addRelation(arB[j], id.succ, Relation.COMMPRED);
						rmRelation.addRelation(id.succ, arB[j], Relation.COMMPRED);
					...
					for (int j = 0; j < arA.length; j++) {
						rmRelation.addRelation(arA[j], id.pred, Relation.COMMSUCC);
						rmRelation.addRelation(id.pred, arA[j], Relation.COMMSUCC);
...
class RelationMatrix {
	...
	public void addRealRelation(String t1, String t2, int rel) {
		TaskUnit tuT1 = (TaskUnit) htRelations.get(t1);
		if (tuT1 == null) {
			tuT1 = new TaskUnit();
			htRelations.put(t1, tuT1);
		}
		tuT1.addRealRela(t2, rel);
	}
	...
	public boolean isShortLoop(String t1, String t2) {
		return (getRelation(t1, t2) & Relation.L2LOOP) > 0;
	}
	...
	public boolean isNormSucc(String t1, String t2) {
		return (getRelation(t1, t2) & Relation.DIRECTCAUSAL) > 0;
	}
	...
	public boolean isNormPara(String t1, String t2) {
		return (getRelation(t1, t2) & Relation.CONCURRENCY) > 0;
	}
	...
	public boolean isParaOrFollow(String t1, String t2) {
		int rel = getRelation(t1, t2);
		return (rel & (Relation.CONCURRENCY | Relation.DIRECTCAUSAL | Relation.REACHABLE)) > 0;
	}
		...
		while (keys.hasMoreElements()) {
			...
			for (int i = 0; i < arPred.length; i++) {
				...
				if (isLoopPara(arPred[i], t)) {
					...
					tu.addPara(arPred[i]);
					// record || relation
					tu.addRela(arPred[i], Relation.CONCURRENCY);
				}
			}
			...
			for (int i = 0; i < arSucc.length; i++) {
				...
				if (isLoopPara(t, arSucc[i])) {
					...
					tu.addPara(arSucc[i]);
					// record || relation
					tu.addRela(arSucc[i], Relation.CONCURRENCY);
				} else if (isBefore(t, arSucc[i])) {
					// increase output tasks
					tu.addSucc(arSucc[i]);
					// record -> relation
					tu.addRela(arSucc[i], Relation.DIRECTCAUSAL);
				}
			}
...
class TaskUnit {
	...
	public void removeShortLoop() {
		...
		while (keys.hasMoreElements()) {
			...
			if ((rel.intValue() & Relation.L2LOOP) > 0) {
				htRela.remove(t);
			}
		}
	}
	...
	public void addRela(String task, int rel) {
		...
		// oldRel = new Integer(rel);
		// store oldRel
		htRela.put(task, oldRel);
	}

	public void addRealRela(String task, int rel) {
		Integer oldRel = (Integer) htRela.get(task);
		if (oldRel == null) {
			oldRel = new Integer(0);
		}
