...
import org.semanticweb.kaon2.ii;
...
*/
	 ...
	 * 对于�?个不可见任务A,如何重构:
	 * 1.判断A中有多少组axyb�?要添加新的place,(无论是x或�?�y都算�?)，记录个数为num
	 * 		1.1 如果num=0,那么不许要split
	 * 		1.2 如果num=1,那么直接添加�?要的place即可,
	 * 		1.3如果num>1,split (num-1)次，每一次分别添加所�?要的place
	 */		
	...
	private void checkAte(AuditTrailEntryList result, int pos) throws IndexOutOfBoundsException, IOException 
	{
		...
		{	
			...
			if (pos == 0)		//第一�?
			{
				if (invTask.type != InvTask.START)	//不是start的话，不予�?�虑
					continue;	
				if (sucTasks.contains(postLogName))	//包含了，�?要添�?
				{
			...
			{
				if (preTasks.contains(postLogName))	//包含了，�?要添�?
				{
					AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
					String invTaskName = invTask.taskName;
					int splitPos = invTaskName.indexOf("\0");
			...
			else
			{
				...
				if (preTasks.contains(preLogName) && sucTasks.contains(postLogName))	//包含了，�?要添�?
				{
					AuditTrailEntryImpl ate = new AuditTrailEntryImpl();
					String invTaskName = invTask.taskName;
					int splitPos = invTaskName.indexOf("\0");
